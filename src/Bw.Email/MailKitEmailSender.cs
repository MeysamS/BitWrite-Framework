...
public class MailKitEmailSender : IEmailSender
{
    ...
    public async Task SendAsync(EmailObject emailObject)
    {
        try
        {
            var email = new MimeMessage { Sender = (MailboxAddress)InternetAddress.Parse(emailObject.SenderEmail ?? _config.From) };
            email.To.Add(InternetAddress.Parse(emailObject.ReceiverEmail));
