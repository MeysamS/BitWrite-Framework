...
using Bw.Extensions.Collection;
using Humanizer;
using MassTransit;
using Bw.Core.Utils;
using IBus = Bw.Core.Messaging.Bus.IBus;
using MessageHeaders = MassTransit.MessageHeaders;
...
public class MassTransitBus : IBus
{
    ...
    public Task Consume<TMessage>(
        Bw.Core.Messaging.MessageHandler<TMessage> subscribeMethod,
        Action<IConsumeConfigurationBuilder>? consumeBuilder = null,
        CancellationToken cancellationToken = default
    )
        where TMessage : class, IMessage
    ...
    void Core.Messaging.Bus.IBusConsumer.Consume<TMessage>(
        IMessageHandler<TMessage> handler,
        Action<Core.Messaging.Bus.IConsumeConfigurationBuilder>? consumeBuilder)
    {
        throw new NotImplementedException();
    }

    Task Core.Messaging.Bus.IBusConsumer.Consume<TMessage>(
        Core.Messaging.MessageHandler<TMessage> subscribeMethod,
        Action<Core.Messaging.Bus.IConsumeConfigurationBuilder>? consumeBuilder,
