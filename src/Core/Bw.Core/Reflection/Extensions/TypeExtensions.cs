...
namespace Bw.Core.Reflection.Extensions;
...
public static class TypeExtensions
{
    ...
    {
        dynamic? awaitable = type.InvokeGenericMethod(methodName, genericTypes, returnType, parameters);
    ...
    {
        dynamic awaitable = type.InvokeMethod(methodName, parameters);
    ...
    )
    {
        ...
        return inputAssemblies.SelectMany(
            assembly => openGenericType.GetAllTypesImplementingOpenGenericInterface(assembly)
        );
    }
    ...
    )
    {
        try
        {
            return openGenericType.GetAllTypesImplementingOpenGenericInterface(assembly.GetTypes());
        }
    ...
    {
        var inputAssemblies = assemblies.Any() ? assemblies : AppDomain.CurrentDomain.GetAssemblies();
        return inputAssemblies.SelectMany(assembly => interfaceType.GetAllTypesImplementingInterface(assembly));
    }
    ...
    public static IEnumerable<Type> FindInterfacesThatClose(this Type pluggedType, Type templateType)
    {
        ...
        foreach (var interfaceType in pluggedType.GetTypeInfo().BaseType!.FindInterfacesThatClose(templateType))
        {
            yield return interfaceType;
        }
    }
